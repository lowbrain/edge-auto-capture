...
実機で埋める残りは `Issue #120` の「未検証（実機で埋める）」が正で、smoke の結果と混同しないこと。
...
- **§1 のキャッシュ削除を飛ばした `pytest` の結果を報告する**（macOS）。古いバイトコードで
  走った緑は「検証していないのに緑」で、このスキルが最も防ぎたいもの。**特に「わざと壊して
  FAIL を確かめた」と報告に書くときは、その FAIL / PASS がキャッシュ削除後のものか必ず確認する**
...
- **CI の再現はしない。** CI は Windows smoke と Python 3.10 / 3.13 マトリクスを回すが、ローカルで真似しない。
