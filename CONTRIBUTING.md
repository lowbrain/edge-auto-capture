...
`pyproject.toml` の `requires-python = ">=3.10"` / ruff の
...
`try_eval` / `_step` / `infra` の各 `except: pass` は
...
### §1-5. バインディング名の出所は `badge.py` の `BIND_*` 1 箇所。`badge.js` に名前は無い（#100）
...
### §1-6. `badge.js` のシャドウは `closed`・呼び出しは `callBinding` 経由
...
### §1-7. `ruff check` / `mypy` は緑（終了コード 0）が正常

**指摘が出たらそれは新しく入れた問題**なので直すこと。

### §1-8. `infra._message_box_windows` の `ctypes.windll` は `Any` 経由の属性アクセスが正
...
### §1-9. コメントに残すのは「罠」。「経緯」は git へ返す

各所の日本語コメントの本体は
落とし穴回避の記録なので、リファクタで関数を移すときは**一緒に運ぶ**（移動のついでに
消して書き直さない）。ただし何を書いてもよいわけではない。判定基準はこの 1 つ:

> その段落を消したとき、**どのテストも落ちず、次に読む人が踏む罠も残らない**なら、
> それは経緯なので git へ返す。

- **残す**（罠・仕様の根拠）: 「この区間に `await` を挟まない」、索引 CSV の BOM の扱い、
  URL キーがフラグメントを落とす理由、`BASE_DIR` が cwd 基準である理由（§1-10）のような、
  知らずに触ると無言で壊すことの説明。
- **落とす**（変更の経緯）: 「以前は 3 箇所へ手書きされていた」「以前は 8 個の位置引数で」
  の類と、実装の理由ではなく**作業の出典**としての Issue 番号（「#55 で」「#81 のときに」）。
  どちらも `git log` / `git blame` が持っている情報で、コメントに置くと二重管理になる
  （冒頭の `docs/ROADMAP.md` と同じ構造。置き場所がコメントなだけ）。

罠は**現在形で書く**（「以前は X だった」ではなく「X にしてはいけない。理由は…」）。
経緯の形で書くと、実装が変わって嘘になったことを誰も検査できない
（ドキュメントの `*.py` 参照には `tests/test_docs_refs.py` という番人がいるが、
散文の内容が実装と合っているかは機械で落とせない）。

### §1-10. `infra.BASE_DIR` の基準は非 frozen 実行では cwd。パッケージフォルダではない（#81）

**`infra.BASE_DIR`（＝ `config.ini` / `output/` / `log.txt` の基準フォルダ）は
