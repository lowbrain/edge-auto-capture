...
class CaptureSession:
    ...
    def __init__(self, context, config: Config) -> None:
        ...
        # Python→ページのヘルパ（applyState/captureStart 等）を収める window プロパティ名。
        ...
        # 撮影 1 回ごとの成否は on_result で受け、撮影カウンタ／失敗把握に使う。
        ...
        # 本セッションで保存できた枚数。動作実感＋暴走の早期発見のため全バーへ配る。
        # 本体はここに持ち、成功のたびに増やして全ページの操作バーへ反映する。
        self.shots = 0
        # 過去に確定したセレクタの履歴。入力欄の datalist 候補として全バーへ配る。
    ...
    async def _on_capture_result(self, ok: bool) -> None:
        """撮影 1 回分の成否を受け、撮影カウンタを更新して全バーへ配る。
    ...
    async def _broadcast(self, call: str) -> None:
        ...
        撮影カウンタ・セレクタ履歴のように「全ページ共通の値」を配る経路の
    ...
    async def _push_history(self) -> None:
        """現在のセレクタ履歴（datalist 候補）を開いている全ページの操作バーへ配る。"""
    ...
    async def on_commit_selector(self, source, token=None, value="") -> None:
        ...
        セレクタ履歴へ積み、変化があれば全バーの入力候補（datalist）を更新する。
    ...
    async def get_state(self, source, token=None) -> dict:
        ...
        枚数表示を正しく初期化する（作り直したバーが 0 枚へ戻って見えないように）。
        ...
        セレクタ履歴（datalist 候補）も同時に返し、遷移後も入力候補を保つ。
        """
        if not self._authorized(token):
            return {
    ...
    async def setup(self) -> None:
        ...
        # framenavigated も拾えるよう、goto より前に張っておく）。
    ...
    async def run(self, closed: asyncio.Event) -> None:
        """ブラウザのウィンドウが閉じるまで待つ（イベント駆動。ポーリング無し）。
...
def _prepare_output_dir(config: Config) -> bool:
    ...
    output_dir は load_config が書き込み可能な場所へ解決済みで、起動単位の
    セッションフォルダ（例: .../output/2026-08-11_143025）まで含んでいる。その後に
...
async def _launch_browser(p, config: Config, user_data_dir: str):
    ...
    空なら Edge→Chrome の順）。起動できたら採用ブラウザの実バージョンをログへ残す（
...
def cli(run: Callable[[Coroutine[Any, Any, None]], None] = asyncio.run) -> int:
    ...
    （多重起動抑止は失敗すると保存先を 2 プロセスで奪い合う、実害のある分岐）。
    ...
    **順序に意味がある**（変えないこと）:
    ...
    # 環境情報と採用設定値を起動時に1行ずつ残す。切り分けが楽になる。
    # ブラウザの実バージョンは起動後に main() 内で別行として出す。
    log(startup_environment_line())
    log(summarize_config(config))

    # 多重起動を入口で止める。二度押しで 2 つ目が起動すると、output/・
    # log.txt・使い捨てプロファイルを先行インスタンスと奪い合うため、
